class ContactsController < ApplicationController
  ...
  # GET /contacts.json
  def index
    @contacts = Contact.all.order(:nome).page(params[:page]).per(15)
  end
  ...
  private
    ...
    def set_options_for_select
      @type_options_for_select = Type.all
    end
    ...
    def contact_params
      params.require(:contact).permit(:nome, :email, :type_id, :rmk, 
      address_attributes: [:street, :city, :state],
      phones_attributes: [:id, :telephone, :_destroy])
    end
end
